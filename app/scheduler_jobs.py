                    ...
                    if reminder_time_utc <= now_utc and start_time_utc > now_utc:
                        ...
                        if not existing_notif:
                            notif = Notification(
                                ...
                                message=message, 
                                # Membuat link spesifik untuk event
                                link=f"/hub#event-{event.id}" 
                            )
                            db.session.add(notif)
                            print(f"  -> MEMBUAT NOTIFIKASI EVENT: {event.title}")

            # 2. Cek Tugas (Tasks)
            tasks_to_remind = db.session.query(Task).filter(Task.reminder_minutes > 0, Task.due_date != None).all()
            if tasks_to_remind:
                    ...
                    if reminder_time_utc <= now_utc and due_date_utc > now_utc:
                        ...
                        if not existing_notif:
                            notif = Notification(
                                ...
                                message=message, 
                                # Membuat link spesifik untuk tugas
                                link=f"/hub#task-{task.id}"
                            )
                            db.session.add(notif)
